...
TEST_F(JacobiSolverTest, SolvesLynnPowellBusWithoutGeneratorTest)
{
    auto model = generateLynnPowellWithoutGenerator();
    model.compile(false);

    fPotencia::Solver_Jacobi solver(model);
    solver.EPS = 1e-12;
    solver.Max_Iter = 100;
...
TEST_F(JacobiSolverTest, SolvesLynnPowellBusWithGeneratorTest)
{
