...
TEST_F(NRcurrentSolverTest, SolvesLynnPowellBusWithoutGeneratorTest)
{
    auto model = generateLynnPowellWithoutGenerator();
    model.compile(false);

    fPotencia::Solver_NRcurrent solver(model);
    solver.EPS = 1e-12;
    solver.Max_Iter = 100;
...
TEST_F(NRcurrentSolverTest, SolvesLynnPowellBusWithGeneratorTest)
{
